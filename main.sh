...
&nam_psop
  ...
  datestring="${analdate}",obsfile="${obs_datapath}/psobs_${analdate}.txt",zthresh=6000,ps_ind=128,
/
EOF
   cat psop.nml
...
&nam_psop
  ...
  datestring="${analdate}",obsfile="${obs_datapath}/psobs_${analdate}.txt",zthresh=6000,
/
EOF
   cat psop.nml
   export PGM="${execdir}/psop_ncio_mslp.x"
   echo "mpitaskspernode = $mpitaskspernode threads = $OMP_NUM_THREADS"
...
sh ${scriptsdir}/run_panguwx.sh > ${current_logdir}/run_panguwx.out 2>&1
errstatus=$?
if [ $errstatus -ne 0 ]; then
